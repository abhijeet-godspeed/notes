...
* When writing a test file, dont change any of the code that was generated using the scaffolding because its absolutely correct. doing changes in the generated code will lead to new errors. you just have to use the comments in the scaffolding as guide to write the tests in that file. just fill the code in the specified empty places. dont try to edit the code that was scaffolded.
* You don't have to write test cases about schema validation because events in Godspeed already handle that.
* Use Mocha + Chai to write test cases.
